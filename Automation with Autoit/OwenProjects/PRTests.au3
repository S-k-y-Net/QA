...
#AutoIt3Wrapper_Res_Fileversion=1.2.1.43
	...
	for $i = 0 to UBound($CSVFiles) - 1
		if UBound($CSVFiles,2) <> 0 Then
			for $j = 0 to UBound($CSVFiles,2) - 1
				$totalTime = 0 ;обнуляем общее время перед подсчетом

				if $CSVFiles[$i][$j] = '' Then ExitLoop

				;Блок подсчета времени необходимое для одного сценария теста
				;-----------------------------------------------------------
				$efile = FileOpen(@ScriptDir&"\"&$CSVFiles[$i][$j], 0)
				if @error = -1 Then
					ConsoleWrite("Can not open "&$CSVFiles[$i][$j])
					Exit
				EndIf
				...
				;блок загрузки проектов в Owen Logic
				;-----------------------------------------------------------------------------------------------------
				...
				;Блок загрузки сценарий тестов в ПЛК и получение результатов
				;------------------------------------------------------------------------------------------------------
